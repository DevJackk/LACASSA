...
lacassa = Token:match("(%d+)")
redis:set(lacassa..":VERSION",1)
redis:set(lacassa..":SUDO_ID:",SUDO_USER)
redis:set(lacassa..":DataCenter:",'German')
redis:set(lacassa..":UserNameBot:",BOT_User)
redis:set(lacassa..":NameBot:",BOT_NAME)
redis:hset(lacassa..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3lacassa_INSTALL","Yes")
info = {}
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس LACASSA بنسبه 100% ]  }•\e[0m"
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @LACASSA_TEAM_3 }• }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE LACASSA source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JACK : @V_P_E  }•\e[0m"

echo -e "\e[38;5;77m      LACASSA ✾ @V_P_Ee[0m"
echo -e "\e[38;5;77m       ✾تم تطوير وبرمجة السورس من قبل جاك ✾ .e[0m"
echo -e "\e[38;5;77m       ✾ @V_P_Ee[0m"
-------------------------------------------------------------------
]]..'\027[0;32m')
create_config() 
else
...
lacassa = Token:match("(%d+)")
our_id = tonumber(lacassa)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(lacassa..":UserNameBot:")
SUDO_ID = tonumber(redis:get(lacassa..":SUDO_ID:"))
SUDO_USER = redis:hgetall(lacassa..'username:'..SUDO_ID).username
version = redis:get(lacassa..":VERSION")
DataCenter = redis:get(lacassa..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..lacassa..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(lacassa..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(lacassa..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(lacassa..'group:add'..msg.chat_id_) then 
...
elseif msg.sender_user_id_ == 839224120 then 
...
msg.Rank = 1
elseif redis:sismember(lacassa..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(lacassa..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(lacassa..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(lacassa..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(lacassa..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(lacassa..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(lacassa..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(lacassa..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(lacassa..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(lacassa..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(lacassa..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(lacassa..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس LACASSA ⇐ @LACASSA_TEAM_3
...
if msg.GroupActive and redis:get(lacassa..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(lacassa..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(lacassa..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(lacassa..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.lacassa then 
if msg.text and ScriptFile.ilacassa then
for k, lacassa in pairs(ScriptFile.lacassa) do
local SearchText = msg.text:match(lacassa)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ilacassa(msg,{msg.text:match(lacassa)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",lacassa.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dlacassa then
if ScriptFile.dlacassa(msg) == false then
return false
end
...
if Plug.lacassa then 
if msg.text and Plug.ilacassa then
for k, lacassa in pairs(Plug.lacassa) do
local SearchText = msg.text:match(lacassa)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(lacassa..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(lacassa..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ilacassa(msg,{msg.text:match(lacassa)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",lacassa.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dlacassa then
Plug.dlacassa(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source lacassa")
end 

end
else
print("The File Script.lua Not Runing in The Source lacassa")
...
function tdcli_update_callback(data)
	...
	redis:srem(lacassa..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/LACASSA/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURC')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/LACASSA/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/LACASSA/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE LAC')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/LACASSA/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/LACASSA/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/LACASSA/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE LACASSA\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس LACASSA بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(lacassa..":SUDO_ID:")
redis:hset(lacassa..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(lacassa..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(lacassa..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(lacassa..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(lacassa..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(lacassa..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(lacassa..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(lacassa..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(lacassa..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(lacassa..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(lacassa..'group:add'..data.chat_.id_) then
	redis:set(lacassa..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(lacassa..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(lacassa..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(lacassa..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	lacassa..'group:name'..k,IDS.Title,
	lacassa..'num_msg_lacassa'..k,5,
	lacassa..'group:add'..k,true,
	lacassa..'lock_link'..k,true,
	lacassa..'lock_spam'..k,true,
	lacassa..'lock_webpage'..k,true,
	lacassa..'lock_markdown'..k,true,
	lacassa..'lock_flood'..k,true,
	lacassa..'lock_bots'..k,true,
	lacassa..'mute_forward'..k,true,
	lacassa..'mute_contact'..k,true,
	lacassa..'mute_document'..k,true,
	lacassa..'mute_inline'..k,true,
	lacassa..'lock_username'..k,true,
	lacassa..'replay'..k,true
	)
	redis:sadd(lacassa..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(lacassa..'username:'..ID,'username',user)
	redis:sadd(lacassa..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(lacassa..'username:'..ID,'username',user)
	redis:sadd(lacassa..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(lacassa..'username:'..ID,'username',user)
	redis:sadd(lacassa..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(lacassa..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(lacassa..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(lacassa..'username:'..data.user_.id_).username
	...
	redis:hset(lacassa..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
